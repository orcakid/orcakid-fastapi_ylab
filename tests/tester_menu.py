...
def test_create_menu2(client, test_db):
    response = client.post(
        '/api/v1/menus',
        json={'title': 'My menu 1', 'description': 'My menu description 1'},
    )
...
def testing_add_dish(client, test_db):
    response = client.post(
        '/api/v1/menus/0/submenus/0/dishes',
        json={
            'title': 'My dish 1',
...
def test_patch_dish(client, test_db):
    response = client.patch(
        '/api/v1/menus/0/submenus/0/dishes/0',
        json={
...
def test_delete_menu2(client, test_db):
    response = client.delete('/api/v1/menus/0')
    assert response.json() == {
        'status': True,
        'message': 'The menu has been deleted',
