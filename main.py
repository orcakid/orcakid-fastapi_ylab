...
def get_one_submenu_of_menu_by_id(menu_id_for_submenu: int, submenu_id: int):
    one_submenu = db.query(models.Submenu).filter(
        models.Submenu.menu_id == menu_id_for_submenu).filter(
...
def delete_submenu(menu_id_for_submenu: int, submenu_id: int):
    s_menu_for_delete = db.query(models.Submenu).filter(
        models.Submenu.menu_id == menu_id_for_submenu).filter(
...
def get_one_dishes(menu_id: int, submenu_id: int, dish_id: int):
    dish = db.query(models.Dish).filter(models.Dish.id == dish_id).filter(
        models.Dish.submenu_id == submenu_id).filter(
